...
$user = $getFromUser->userData($use_id);
    ...
    if(!empty($currentPwd) && !empty($newPassword) && !empty($rePassword)){
        if($getFromUser->checkPassword($currentPwd) === true){
            if(strlen($newPassword) < 6){
                ...
                $error['rePassword'] = "password doesn't not match";
            }else {
                $getFromUser->update('users', $use_id, array('password' => md5($newPassword)));
                header('Location:'.BASE_URL. $user->username);
            }
        }else {
            $error['currentPwd'] = "Password is in correct";
        }
    }else {
                        ...
                        <li class="hover"><label class="drop-label" for="drop-wrap1"><img src="<?php echo $user->profileImage; ?>"/></label>
                            ...
                            <div class="drop-wrap">
                                <div class="drop-inner">
                                    <ul>
                                        <li><a href="<?php echo BASE_URL.$user->username; ?>"><?php echo BASE_URL.$user->username; ?></a></li>
